...
import { streamAgentTurn, type CursorEvent } from "../src/provider/agent-events.js";

const MESSAGE: SDKUserMessage = { type: "user", text: "hi" } as unknown as SDKUserMessage;

type OnDelta = (input: { update: Record<string, unknown> & { type: string } }) => void;
...
/** Build a fake SDKAgent whose send() drives onDelta and resolves wait(). */
function fakeAgent(opts: {
  ...
  return {
    agentId: "agent-test",
    send: async (_message: SDKUserMessage, sendOptions?: Record<string, unknown>) => {
      ...
      const run: Partial<Run> = {
        wait: async () => (opts.result ?? { status: "finished", result: "" }) as never,
        cancel: async () => {},
      };
      return run as Run;
    },
...
async function collect(events: AsyncGenerator<CursorEvent>): Promise<CursorEvent[]> {
...
describe("streamAgentTurn run terminal status", () => {
  it("throws when the run ends with status 'error' instead of finishing silently", async () => {
    ...
    await expect(collect(streamAgentTurn(agent, MESSAGE, { mode: "agent" }))).rejects.toThrow(
      /error/i,
    );
  });
...
describe("streamAgentTurn busy-agent recovery", () => {
  ...
  it("does not retry non-busy send failures", async () => {
    ...
    const sendCalls: Array<Record<string, unknown> | undefined> = [];
    const agent = fakeAgent({ rejectFirst: { error: nope, times: 99 }, sendCalls });
      ...
      updates: [
        {
          ...
          toolCall: {
            ...
            result: { status: "success", value: { content: [], isError: true } },
          },
        },
      ],
